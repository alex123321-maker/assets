...
Новые пакеты используют [Gemini workflow](docs/GEMINI_WORKFLOW.md) и
[quality gate](docs/QUALITY_GATE.md): сначала один визуальный эталон,
затем семейство; build evidence привязано к исходникам, visual review записывается
отдельно после просмотра изображений. Для браузерного PR review есть генератор
пакета ссылок на конкретный commit. [Уроки прошлых PR](docs/PR_REVIEW_LESSONS.md).
