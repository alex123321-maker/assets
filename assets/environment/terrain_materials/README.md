...
| **Cliff / Rock Side** | Скальные обрывы (боковые грани) | 16×16 | 0.92 | `#44413e`, `#524e4a`, `#746e67` | Темнее верха; нерегулярные горизонтальные пласты со ступенчатыми сколами; глубина перепадов рельефа. |
...
### Координаты регионов (UV Bounding Boxes):
...
Каждый showcase-блок является валидным пакетом `voxel_static` и содержит `manifest.json`, `request.md`, `source/voxels.json`, `output/model.glb` и `review/` с 4 ортогональными рендерами (iso, front, side, top 512×512).
