...
Greedy meshing не включён в baseline, потому что объединение больших поверхностей может менять читаемость блочной структуры. Добавлять его только если profiler показывает необходимость.
