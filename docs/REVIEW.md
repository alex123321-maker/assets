...
При before/after проверь `comparison.json`: одинаковый тип камеры не означает одинаковый
масштаб. Не принимай оценочные подписи на сгенерированном листе за независимое доказательство.
Читай неподписанные renders до самоотчёта автора. Наблюдения должны описывать видимое,
а не устройство генератора; при споре «5 clusters» не доказывают «5 видимых масс».

Пакет с меткой DRAFT допускает явные fail/not_reviewed и не является заявкой на merge.
Не требуй от исполнителя подменить статус ради получения пакета ревью.
