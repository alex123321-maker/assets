...
Это относится и к подписям на PNG/contact sheets: генератор рисует нейтральные названия,
измерения или вопросы для reviewer, но не художественные выводы и не статус APPROVED.
Сначала опиши видимое на картинке без чтения комментариев генератора, затем сравни с критерием.
Если критерий не выполнен — запиши fail; для раннего внешнего review используй `packet --draft`.
Чистую воспроизводимость проверяй `quality_gate.py verify-clean <package>`.
